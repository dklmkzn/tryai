...
app.get('/process', async (req, res) => {
    ...
    try {
        const content = await yandex.extractTextFromYaRu(shortUrl, config.YANDEX_TOKEN, tgUtils.logMessage, adminChatId, bot, config.DIAGNOSTIC_MODE);
...
function startPingScheduler() {
    ...
    function doPing() {
        axios.get(`${RENDER_URL}/ping`).catch(err => {
            // Критическая ошибка — будет обработана через logMessage, но здесь мы не можем вызвать logMessage без параметров
            // Оставляем console.error, так как это критическая ошибка и она попадёт в обработку через logMessage, если вызвать её позже.
